...
Thread::Thread(const char* threadName, bool flag, int prior)
{
    name     = threadName;
    stackTop = NULL;
    stack    = NULL;
...
void
Thread::ModifyPriority(int prior)
{
    priority = prior;
}
...
void
Thread::CloseFile(OpenFileId id)
{
    if (id>1 && id<MAX_OPEN_FILES)
        files[id] = NULL;
}
