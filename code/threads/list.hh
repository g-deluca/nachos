...
template <class Item>
void
List<Item>::FindAndRemove(Item anotherItem)
{
    ListNode *ptr;
    if (first != NULL && first->item == anotherItem){
        first = first->next;
    } 
    for (ptr = first; ptr != NULL && ptr->next != NULL; ptr = ptr->next) {
        if ((ptr->next)->item == anotherItem)
