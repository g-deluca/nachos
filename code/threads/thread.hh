...
public:
    ...
    /// Initialize a `Thread`. A flag is added to specify if the thread 
    /// is joinable or not.
    Thread(const char *debugName, bool flag,int priority);
    ...
    /// Block the thread until another thread finishes
    int Join();
    ...
    /// The thread verify if it is joinable
    bool IsJoinable();
...
private:
    ...
    /// A port that will be used if a thread uses the join method
    Port *port;
    bool joinFlag;

    /// The priority that the process has in the scheduler
    int priority;
    int realPriority;
...
public:
    ...
    // Add a file and generate an id
    OpenFileId AddFile (OpenFile* f);
    
    // Close a file
    void CloseFile (OpenFileId id);

    // Fetch and open file from a required id
